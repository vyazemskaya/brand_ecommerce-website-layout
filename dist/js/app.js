    ...
    (() => {
        ...
        function menuInit() {
            if (document.querySelector(".icon-menu")) document.addEventListener("click", (function(e) {
                ...
                if (bodyLockStatus && (e.target.closest(".menu__close") || !e.target.closest(".menu__body"))) {
                    bodyUnlock();
        ...
        function filterMenuInit() {
            const filterMenuBody = document.querySelector(".filter-head-collection__body");
            if (filterMenuBody) document.addEventListener("click", (function(e) {
                if (bodyLockStatus && e.target.closest(".filter-head-collection__button")) {
                    bodyLock();
                    filterMenuBody.classList.add("_active");
                }
                if (bodyLockStatus && e.target.closest(".filter-head-collection__button-close")) {
                    bodyUnlock();
                    filterMenuBody.classList.remove("_active");
                }
            }));
        }
        function showMore() {
        ...
        function initSliders() {
            ...
            if (document.querySelector(".offers-mainpage__slider")) if (screenWidth < 991.98 && !offersSlider) offersSlider = new core(".offers-mainpage__slider", {
                ...
                speed: 800,
                navigation: {
                    nextEl: ".offers-mainpage__navigation .navigation__arrow_next",
                    prevEl: ".offers-mainpage__navigation .navigation__arrow_prev"
                },
                pagination: {
                    el: ".offers-mainpage__pagination",
                    type: "bullets",
                    clickable: true
                },
                breakpoints: {
            ...
            }); else if (screenWidth > 991.98 && offersSlider) {
                offersSlider.navigation.destroy();
                offersSlider.destroy();
                offersSlider = null;
            }
            if (document.querySelector(".suggestions-mainpage__slider")) if (screenWidth < 767.98 && !suggestionsSlider) suggestionsSlider = new core(".suggestions-mainpage__slider", {
                ...
                speed: 800,
                pagination: {
                    el: ".suggestions-mainpage__pagination",
                    type: "bullets",
                    clickable: true
                },
                breakpoints: {
        ...
        da.init();
        window.addEventListener("load", (function(e) {
            const submenusInit = target => {
                const submenusContainer = document.querySelector(".menu__content");
                const submenuOpen = target => {
                    const submenuOpenBtnId = target.getAttribute("data-menu-id");
                    const submenuActiveItem = submenusContainer.querySelector(`.menu__catalog[data-menu-id="${submenuOpenBtnId}"]`);
                    if (submenuActiveItem) submenusContainer.querySelector(`.menu__catalog[data-menu-id="${submenuOpenBtnId}"]`).classList.add("_active");
                };
                const submenuClose = target => {
                    target.closest(".menu__catalog").classList.remove("_active");
                };
                if (target.closest(".list-menu__item_parent")) submenuOpen(target); else if (target.closest(".menu__back-btn")) submenuClose(target);
            };
            document.addEventListener("click", (function(event) {
                const target = event.target;
                console.log(bodyLockStatus);
                if (target.closest(".menu__catalog")) submenusInit(target);
                if (target.closest(".item-products-collection__button")) event.preventDefault();
            }));
        }));
        window["FLS"] = true;
        isWebp();
